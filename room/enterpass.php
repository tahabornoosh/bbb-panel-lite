...
if(mnnv($_POST['email']) && mnnv($_POST['password'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            if(md5($mresp) == $row['pass'] && $row['visible'] == 1) {
                ...
                redirect('login.php?id='.$_POST['roid']);
            }
            else if($row['pass'] != md5($mresp)) {
                redirect('login.php?id='.$_POST['roid'].'&mas=Password is Wrong');
            }
            else if($row['visible'] != 1) {
                redirect('login.php?id='.$_POST['roid'].'&mas=Email not Found');
            }
        }
        } else {
            redirect('login.php?id='.$_POST['roid'].'&mas=Email not Found');
    ...
    redirect('login.php?id='.$_POST['roid'].'&mas=Email and password are reqired');
}
?>